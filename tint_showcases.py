...
# Multicolor ("Mix") palettes, read directly from AppStore-Collections.json.
# These collections are NOT naturalHueTint, so the app renders them with
# kCGBlendModeMultiply at tintAlpha .82 -- reproduced here pixel-for-pixel.
MULTICOLOR = {
    "everyday-joy": ["#E94355", "#55B95D", "#FFD447", "#F080B6", "#55A9E8", "#FFF0CE"],
    "haunted-glow": ["#A60016", "#FF6A00", "#8B35D1", "#39FF14", "#F2E9D8"],
    "autumn-harvest": ["#A83F25", "#416B3A", "#D7A61D", "#C96546", "#FFF0CE", "#F0982D"],
    "celestial-dreams": ["#D74CA8", "#7139C6", "#FFD166", "#254A9B", "#F5F4FF", "#42C7D7", "#73E0B1"],
}
TINT_ALPHA = 0.82
...
def tint_bulb(image, hex_color, alpha=TINT_ALPHA):
    """Reproduces the app's kCGBlendModeMultiply tint at the given alpha."""
...
def make_showcase(collection_id, images, built_in=False, glow_colors=None):
...
for collection_id, palette in MULTICOLOR.items():
    ...
    make_showcase(collection_id, tinted, built_in=bool(collection.get("builtInReceptacle")), glow_colors=glow_colors)
    print(f"Retinted {collection_id} with {len(images)} bulbs, multicolor")
